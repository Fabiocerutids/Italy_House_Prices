...
df = pd.read_csv('../data/sale_data/2_sale_enriched.csv')
st.markdown('''
...
new_input_dict['citta'] = st.selectbox('Città',sorted(list(df[df['regione']==new_input_dict['regione']]['citta'].unique()))) 
...
if st.button('Consigliami il prezzo di vendita'):
    # Set the content type
    headers = {"Content-Type": "application/json"}
    resp = requests.post(docker_url, input_data, headers=headers)
    price_prediction = json.loads(resp.text)['Prediction']
    st.markdown(f'Per la proprietà data in input, consiglio un prezzo di vendita di {price_prediction}€')